...
$reg=mysqli_real_escape_string($con,$_POST['regcode']);

if ($reg=="") {
    header("location:https://cyber.rit.edu/SCRRPDT/hello.php?eid=1/");
    exit();
}


#CHECK FOR COMPANY EXISTS
$sql="SELECT regcode FROM member_info WHERE uid = '".$_SESSION['un']."' ;";
$result = mysqli_query($con,$sql) or die("Query fail: " . mysqli_error($con));
while ($row = mysqli_fetch_array($result)) {
    $regcode = $row['regcode'];
    if ($regcode==$reg) {
        $sql="UPDATE member_info SET registered='1' WHERE uid ='".$_SESSION['un']."';";
        $result = mysqli_query($con,$sql) or die("Query fail: " . mysqli_error($con));

        header("location:https://cyber.rit.edu/SCRRPDT/secure/");
        exit();
    }
}
...
$messagebody =
    ...
    '<title>SCRR Registration Complete</title>' . "\n" .
    '<body style=text-align:center;>' . "\n" .
    '<h1>Hi, ' . $_SESSION['fn'] . '! </h1>' . "\n" .
    '<h3>You\'ve been registered with SCRRPDT!<br>To validate your email, we\'ve assigned you this one-time registration code</h3>'. "\n" .
    '<h1>' . $regcode . '</h1>' . "\n" .
