...
$m=array_key_exists('m', $_REQUEST) ? $_REQUEST['m'] : null;
?>
...
<div class="main">
    ...
    <form method="POST" action="updateobjectives.php">
        <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12 xs-12">
                <?php if (!isset ($g )) {?>
                    <span class="nist-title">Select a Goal</span>
                    ...
                    while ($row = mysqli_fetch_array($result)) {
                        ...
                        if ($goal_no <> "") {
                            ?>
                            <option value="objectives.php?g=<?php echo $goal_no; ?>"><?php echo $goal_no.". ".$goal; ?></option>
                        <?php }}
                    ?>
                ...
                <?php } else
                { ?>
                    <span class="nist-title">Goal:</span>
                <?php
...
<!-- Once goal is selected, bring in the Objectives here... -->
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 xs-12">
                <span class="nist-title">Our Objectives  <?php if ($m==1) {echo " - Updated!";}?></span>
                <select id="objective" size="<?php echo $objectivesize; ?> " style="width:100%;">
                    <?php
                    foreach ($objectives as $v) {
                        ?>
                        <option value="<?php echo $v['objective']; ?>" data-id="<?php echo $v['objective_no']; ?>">
                            <?php echo $v['objective_no'].": ".$v['objective']; ?></option>
                        <?php
                    }
                    ?>
                </select>
                <input id="objective_count" type="hidden" name="objective_count" value="<?php echo $objectivesize; ?>">
                <input id="objective_id" type="hidden" name="objective_id" value="">
                <input id="objective_code" type="hidden" name="objective_code" value="add">
                <input id="objectiveinput" type="text" name="objectiveinput" value="" placeholder="Add a New Objective" style="display:block;width:100%" onfocus="myMInputFunction()">
                <div class="row">
                    <div class="col-lg-4 col-md-4 text-left">
                    <button id="objectivebtn" type="submit" class="btn-info btn-sm text-left">Add New Objective</button>
                    </div>
                    <div class="col-lg-4 col-md-4 text-center ">
                    <button id="objectiveedit" type="submit" class="btn-default btn-sm text-center" style="display:none;">Edit</button>
                    </div>
                    <div class="col-lg-4 col-md-4  text-right">
                    <button id="objectivedelete" type="submit" class="btn-danger btn-sm text-right" style="display:none;"  onclick="myDeleteFunction()">Delete</button>
                    </div>
                </div>
            </div>
        </div>
